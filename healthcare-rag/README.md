...
> **Note on LLM Provision**:
> By default, the system will use a local Mock LLM if no API key is provided, returning hardcoded responses. For full capabilities, copy the placeholder `.env` file to `.env` in the root folder and add your `ANTHROPIC_API_KEY`:
> ```env
...
2. Observe the sidebar and upload the file from `data/sample_medical_guidelines.txt` using the file uploader. Uploading triggers backend processing with `tiktoken` limit chunking (500 tokens).
3. Try asking an in-scope question: `"What should I eat after an appendectomy?"`
4. Try asking an out-of-scope question: `"How do I treat a broken arm?"` 
